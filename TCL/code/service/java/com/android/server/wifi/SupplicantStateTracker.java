...
class SupplicantStateTracker extends StateMachine {
    ...
    private int mWEPObtainIPFailuresCount = 0; //[BUGFIX]-Add by TCTNB.ruili.liu,12/13/2014,PR854538, Limit device try to connect  WEP AP times
    ...
    private void handleWepObtainIPFailure() {
        Message message = getCurrentMessage();
        if (DBG) {
            Log.d(TAG, "handleWepObtainIPFailure wep ap = " + message.arg1 + " nid = " + message.arg2
                    + " mWEPObtainIPFailuresCount = " + mWEPObtainIPFailuresCount);
        }
        if (message.arg1 == 1) {
            mWEPObtainIPFailuresCount++;
            mWifiConfigStore.enableNetworkWithoutBroadcast(message.arg2, true);//PR882717 ADISON.XU ADD
        } else {
            mWEPObtainIPFailuresCount = 0;
        }
        if (mWEPObtainIPFailuresCount >= 3) {
            int nid = message.arg2;
            WifiConfiguration config = mWifiConfigStore.getWifiConfiguration(nid);
            config.setAutoJoinStatus(WifiConfiguration.AUTO_JOIN_DISABLED_ON_AUTH_FAILURE);
            mWifiConfigStore.disableNetwork(nid, WifiConfiguration.DISABLED_AUTH_FAILURE);
            mWEPObtainIPFailuresCount = 0;
        }
    }
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case WifiMonitor.AUTHENTICATION_FAILURE_EVENT:
                    ...
                    handleWepObtainIPFailure(); //[BUGFIX]-Add by TCTNB.ruili.liu,12/13/2014,PR854538, //Limit device try to connect  WEP AP times
                    break;
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    mAuthenticationFailuresCount = 0;//[BUGFIX]-MOD-BEGIN by Adison.Xu, 2015.03.03, if end user trigger a connection ,reset auth fail count, PR933319
                    break;
