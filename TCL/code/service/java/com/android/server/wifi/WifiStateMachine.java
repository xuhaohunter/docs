...
public class WifiStateMachine extends StateMachine {
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.Zhang Jinbo,10/30/2014,720095
    //Add WiFi WPS Push Button Feature
    private TctSoftapMonitor mSoftapMonitor;
    //[FEATTURE]-Add-END by TCTNB.Zhang Jinbo
    ...
    */
    ...
    //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802
    private final WifiPriority mWifiPriority;
    //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802
   ...
   /* Set the frequency band */
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.Zhang Jinbo,10/30/2014,720095
    //Add WiFi WPS Push Button Feature
    private State mApWpsRunningState = new ApWpsRunningState();
    //[FEATTURE]-Add-END by TCTNB.Zhang Jinbo
    ...
    public WifiStateMachine(Context context, String wlanInterface,
        ...
        //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802
        mWifiPriority = new WifiPriority(context,mWifiConfigStore);
        //[BugFix] Add-End by TCTNB.Zhang Jinbo,2014.08.13,PR758802
        //[FEATURE]-Add-BEGIN by TCTNB.Zhang Jinbo,10/30/2014,720095
        //Add WiFi WPS Push Button Feature
        mSoftapMonitor = new TctSoftapMonitor(mInterfaceName, this);
        //[FEATTURE]-Add-END by TCTNB.Zhang Jinbo
        ...
        //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802  841	1018
        mContext.registerReceiver(
                new BroadcastReceiver() {
                    @Override
                    public void onReceive(Context context, Intent intent) {
        ...
        addState(mDefaultState);
            ...
            addState(mSoftApStartedState, mDefaultState);
                ...
                //[FEATURE]-Add-BEGIN by TCTNB.Zhang Jinbo,10/30/2014,720095
                //Add WiFi WPS Push Button Feature
                addState(mApWpsRunningState, mSoftApStartedState);
                //[FEATTURE]-Add-END by TCTNB.Zhang Jinbo
    ...
    //[BUGFIX]-Add-BEGIN by TCTNB.Chen Ji,11/03/2014,553,Wi-Fi FTM dev
    public boolean tct_testModeStartTx(){
        return mWifiNative.testModeStartTx();
    }

    public boolean tct_testModeStopTx() {
        return mWifiNative.testModeStopTx();
    }

    public boolean tct_setWifiTestModeEnabled(boolean enabled) {
        if (enabled) {
            return mWifiNative.loadTestModeDriver();
        } else {
            return mWifiNative.unloadTestModeDriver();
        }
    }
    ...
    private void setWifiState(int wifiState) {
        ...
        //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802
        if(mOperationalMode == SCAN_ONLY_WITH_WIFI_OFF_MODE && wifiState == WIFI_STATE_DISABLED)
        {
            mWifiPriority.sortPriority();
        }
        //[BugFix] Add-End by TCTNB.Zhang Jinbo,2014.08.13,PR758802
    ...
    private void calculateWifiScore(WifiLinkLayerStats stats) {
        ...
        if (score != mWifiInfo.score) {
            ...
            if (!isPropFeatureEnabled) {
                ...
                if(mNetworkAgent != null) {
                    //[BUFFIX] ADD-BEGIN TCTNB.Ruili.Liu CR951413 prohibit wifi calculate mechanism
                    if (mContext.getResources().getBoolean(com.android.internal.R.bool.feature_calculateWifiScore_on)) {
                        mNetworkAgent.sendNetworkScore(score);
                    } else {
                        mNetworkAgent.sendNetworkScore(100);
                    }
                    //[BUGFIX] ADD-END TCTNB.Ruili.Liu
                }
            }
        }
    ...
    private boolean clearIPv4Address(String iface) {
        try {
            ...
            return false;
        //[BUGFIX]-Add-BEGIN by TCTNB.ji.chen,04/07/2015,967194,
        //IllegalStateException handle
        } catch (IllegalStateException ie) {
            loge("Unable to set interface config: " + ie);
            return false;
        }
        //[BUGFIX]-Add-END by TCTNB.ji.chen
    }
    ...
    private void handleNetworkDisconnect() {
        ...
        //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.11.04,PR736784
        if(mLastNetworkId != WifiConfiguration.INVALID_NETWORK_ID)
        {
            List<WifiConfiguration> configs = mWifiConfigStore.getConfiguredNetworks();
            for (WifiConfiguration xconfig : configs) {
                if (xconfig.networkId == mLastNetworkId) {
                    mWifiPriority.updatePriority(xconfig);
                }
            }
        }
        //[BugFix] Add-End by TCTNB.Zhang Jinbo,2014.11.04,PR736784
    ...
    class SupplicantStartedState extends State {
        ...
        public void enter() {
            ...
            int defaultInterval = mContext.getResources().getInteger(
                    R.integer.tct_config_wifi_supplicant_scan_interval);
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    //[BugFix] Add-Start by TCTNB.Zhang Jinbo,2014.08.13,PR758802
                    if(mLastNetworkId != WifiConfiguration.INVALID_NETWORK_ID && mLastNetworkId != netId)
                    {
                        List<WifiConfiguration> configs = mWifiConfigStore.getConfiguredNetworks();
                        for (WifiConfiguration xconfig : configs) {
                            if (xconfig.networkId == mLastNetworkId) {
            ...
            switch (message.what) {
              ...
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                  ...
                  if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS) {
                      ...
                      //[BUGFIX]-Add-BEGIN by TCTNB.ruili.liu,12/13/2014,PR854538,Limit device try to connect  WEP AP times
                      WifiConfiguration config = getCurrentWifiConfiguration();
                      if (config.wepKeys[0] != null) {
                          mSupplicantStateTracker.sendMessage(WifiMonitor.AUTHENTICATION_FAILURE_EVENT,1,config.networkId);
                      }
                ...
                case CMD_START_SCAN:
                    ...
                    //[BUGFIX]-MOD-BEGIN by TCTSH.siwei.xu, 01/28/2015, 905284,
                    //Avoid location wifi scan when doing wifi throughput test
                    } else if (message.arg1 != UNKNOWN_SCAN_SOURCE && message.arg1 != ADD_OR_UPDATE_SOURCE) {
                        if (mWifiInfo.txSuccessRate >
                                mWifiConfigStore.maxTxPacketForPartialScans
                                || mWifiInfo.rxSuccessRate >
                                mWifiConfigStore.maxRxPacketForPartialScans) {
                        ...
                        handleScanRequest(WifiNative.SCAN_WITHOUT_CONNECTION_SETUP, message);
                    //[BUGFIX]-MOD-END by TCTSH.siwei.xu
                    } else {
    ...
    class DisconnectedState extends State {
        ...
        public void enter() {
            ...
            // Reenable all networks, allow for hidden networks to be scanned
            //mWifiConfigStore.enableAllNetworks();//[BUGFIX]-MOD-BEGIN by Adison.Xu, 2015.03.03, when connecting AP with wrong password,enableAllNetworks operation will effect the mAuthentication Failur statistics in supplicantStateTracker, PR933319
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.Zhang Jinbo,10/30/2014,720095
    //Add WiFi WPS Push Button Feature
    class ApWpsRunningState extends State {
        private Message mSourceMessage;
        @Override
        public void enter() {
            mSourceMessage = Message.obtain(getCurrentMessage());
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
                case TctSoftapMonitor.WPS_SUCCESS_EVENT:
                    replyToMessage(mSourceMessage, WifiManager.WPS_COMPLETED);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
                    transitionTo(mSoftApStartedState);
                    break;
                case TctSoftapMonitor.WPS_OVERLAP_EVENT:
                    replyToMessage(mSourceMessage, WifiManager.WPS_FAILED,
                            WifiManager.WPS_OVERLAP_ERROR);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
                    transitionTo(mSoftApStartedState);
                    break;
                case TctSoftapMonitor.WPS_FAIL_EVENT:
                    replyToMessage(mSourceMessage, WifiManager.WPS_FAILED, message.arg1);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
                    transitionTo(mSoftApStartedState);
                    break;
                case TctSoftapMonitor.WPS_TIMEOUT_EVENT:
                    replyToMessage(mSourceMessage, WifiManager.WPS_FAILED,
                            WifiManager.WPS_TIMED_OUT);
                    mSourceMessage.recycle();
                    mSourceMessage = null;
