...
class WifiApConfigStore extends StateMachine {
    ...
    private void setDefaultApConfiguration() {
        WifiConfiguration config = new WifiConfiguration();
        //[FEATURE]-Add-BEGIN by TCTNB.Ruili.Liu,10/20/2014,720205,
        //Wi-Fi hotspot SSID and password should be differ with each other
        String randomUUID = UUID.randomUUID().toString();
        boolean isDiffSSID = mContext.getResources().getBoolean(com.android.internal.R.bool.feature_tctfw_apConfigDiffer_on);
        if (isDiffSSID){
            String preSsid = mContext.getString(com.android.internal.R.string.def_tctfw_settings_wifiap_ssid);
            if (preSsid.length() > 27){
                preSsid = preSsid.substring(0, 27);
            }
            config.SSID = preSsid + " " + randomUUID.substring(24, 28);
        } else {
            config.SSID = mContext.getString(com.android.internal.R.string.def_tctfw_settings_wifiap_ssid);
            if (config.SSID.length() > 32) {
                config.SSID = config.SSID.substring(0, 32);
            }
        }
