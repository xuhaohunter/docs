...
public class WifiAutoJoinController {
    ...
    void attemptAutoJoin() {
        ...
        for (WifiConfiguration config : list) {
            ...
            logDbg("==> check " + config.configKey(true));
            ...
            // [BUGFIX]-add-begin by TCTSH.siwei.xu, 943041.
            // fix phone still attempt to join a authentication method changed AP.
            // Skip authentication method changed AP.
            boolean authChanged = false;
            for(ScanResult result : config.scanResultCache.values()) {
                String configKey = WifiConfiguration.configKey(result);
                if( ! TextUtils.equals(config.configKey(true), configKey) ) {
                    logDbg("attemptAutoJoin skip authentication method changed AP, "
                            + ", " + config.configKey(true) + ", " + configKey);
                    authChanged = true;
                }
            }
            if(authChanged) continue;
            // [BUGFIX]-add-end by TCTSH.siwei.xu, 943041.
