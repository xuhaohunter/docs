    ...
    class FrequencyConflictState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case WifiMonitor.P2P_GO_NEGOTIATION_SUCCESS_EVENT:
                case WifiMonitor.P2P_GROUP_FORMATION_SUCCESS_EVENT:
                    //[BUGFIX]-Add by TCTNB.ruili.liu,11/05/2014,FR728845,TV LINK ergo dev
                    Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.TCT_P2P_DEVICES_CONNECTED, 1);
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.REMOVE_GROUP:
                    ...
                    if (mWifiNative.p2pGroupRemove(mGroup.getInterface())) {
                        transitionTo(mOngoingGroupRemovalState);
                        //[BUGFIX]-Add by TCTNB.ruili.liu,11/05/2014,FR728845,TV LINK ergo dev
                        Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.TCT_P2P_DEVICES_CONNECTED, 0);
                        replyToMessage(message, WifiP2pManager.REMOVE_GROUP_SUCCEEDED);
                    } else {
                        handleGroupRemoved();
                        transitionTo(mInactiveState);
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.TCT_P2P_DEVICES_CONNECTED, 1);
                    break;
                //[BUGFIX]-MOD-BEGIN by TCTNB.ZhangJie,11/24/2014,843703,
                //Solve can not cancel invite when as group owner
                case WifiP2pManager.CANCEL_CONNECT:
                    mWifiNative.p2pCancelConnect();
                    sendP2pConnectionChangedBroadcast();

                    boolean peersChanged = mPeers.remove(mPeersLostDuringConnection);
//[BUGFIX]-MOD-BEGIN by TCTNB.Ji.chen,2014/12/24,PR882199,when disconnect wireless display,DUT reboot
                    if (mSavedPeerConfig.deviceAddress != null && (!TextUtils.isEmpty(mSavedPeerConfig.deviceAddress)) &&
