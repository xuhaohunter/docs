				     ...
				     char *buf, size_t buflen)
{
	...
	// [BUGFIX]-Add-Begin by TCTSH.siwei.xu, 2015/02/04, 920813, WLAN driver do not support country code EU.
#ifdef DRIVER_NOT_SUPPORT_COUNTRY_CODE_EU
	if (os_strncasecmp(cmd, "COUNTRY", 7) == 0) {
		// qcom wlan driver not supported country code EU, we use the same domain country code replaced.
		if(cmd[8] == 'E' && cmd[9] == 'U') {
			cmd[8] = 'F';
			cmd[9] = 'R';
			wpa_printf(MSG_INFO, "nl80211: country code EU not support, replaced by %c%c.", cmd[8], cmd[9]);
		}
	}
#endif
	// [BUGFIX]-Add-End by TCTSH.siwei.xu, 2015/02/04, 920813
