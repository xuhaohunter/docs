...
//[BugFix]Add By TCTSH_Liang Dashun,2014.12.31,PR883249
#include <linux/if.h>
static int EAP_MTU_SIZE = 1500;
static int eap_kif_get_mtu( )
{
  int fd;
  struct ifreq if_mtu;
  ...
  if ( fd < 0 ) {
    wpa_printf(MSG_ERROR,"[Liang:]eap_kif_get_mtu: socket open failed [%d]", fd);
    return -1;
  }
  ...
  if ( ioctl(fd, SIOCGIFMTU, &if_mtu) < 0 ) {
    wpa_printf(MSG_ERROR,"[Liang:]eap_kif_get_mtu: ioctl failed to get mtu");
    close(fd);
    return -1;
  }
  wpa_printf(MSG_INFO,"[Liang:]eap_kif_get_mtu: mtu=[%d]", if_mtu.ifr_mtu);
  close(fd);
  return if_mtu.ifr_mtu;
}

static int eap_kif_set_mtu(unsigned int mtu)
{
    int fd;
   struct ifreq ifr;
   const char *ifname = "wlan0";
   ...
   if( (fd = socket( AF_INET, SOCK_DGRAM, 0 )) < 0 ) {
	 wpa_printf(MSG_ERROR,"[Liang:]eap_kif_set_mtu: socket open failed [%d]", fd);
	 return -1;
   }
   ...
   if( ioctl( fd, SIOCSIFMTU, &ifr ) < 0 ) {
	wpa_printf(MSG_ERROR,"[Liang:]eap_kif_set_mtu: ioctl failed to get mtu");
	 close(fd);
	 return -1;
   }
   wpa_printf(MSG_INFO,"[Liang:]eap_kif_set_mtu: mtu=[%d]", mtu);
   close(fd);
   return 0;
}
//[BugFix]Add By TCTSH_Liang Dashun,2014.12.31,PR883249
...
SM_STATE(EAP, IDENTITY)
{
	...
	//[BugFix]Add By TCTSH_Liang Dashun,2014.12.31,PR883249
	if (EAP_MTU_SIZE!=eap_kif_get_mtu( ))
		eap_kif_set_mtu( EAP_MTU_SIZE);
	//[BugFix]Add By TCTSH_Liang Dashun,2014.12.31,PR883249
	eapReqData = eapol_get_eapReqData(sm);
